...
class EdgeFeatureExtractor:
    ...
    def _edge_stats(self, gray_img):
        ...
        feature_dict["grad_mean"] = safe_mean(grad_mag)
        feature_dict["grad_std"] = safe_std(grad_mag)
        feature_dict["grad_energy"] = float(np.mean(grad_mag ** 2)) if grad_mag.size else np.nan
        feature_dict["laplacian_var"] = safe_var(lap)
        feature_dict["edge_density"] = float(np.mean(edge_pixels)) if edge_pixels.size else np.nan
        ...
        if np.any(edge_pixels):
            ...
            feature_dict["edge_orientation_entropy"] = safe_entropy(angle_hist)
        else:
            feature_dict["edge_orientation_entropy"] = np.nan
